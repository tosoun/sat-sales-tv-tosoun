...
PAPPAS_KEYWORDS = ["ελασσονα", "φαρσαλα", "καρδιτσα", "τρικαλα", "παππας"]
...
SKIADOPOULOS_KEYWORDS = ["κερκυρα", "κέρκυρα", "σκιαδοπουλος"]
...
def filter_dataframe(df_stores):
  ...
  if active_filter == "τομεας 3" or active_filter == "τομέας 3":
    total_sum = df_stores["Num_Sales"].sum()
    return df_stores.reset_index(drop=True), total_sum
